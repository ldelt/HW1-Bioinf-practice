...
3. **Загружаем сессию `ref_and_sorted_bam.xml`**

5. **В пункте `File` > `Open from file` добавляем `VarScan_results.vcf` и исходный файл с аннотацией `GCF_000005845.2_ASM584v2_genomic.gff`, лежащий в папке raw.** Сессия сохранена в файле `manual_pred.xml`

7. **Пытаемся предсказывать эффект каждой мутации** *~~(В этот пункт надо будет что-то более внятное вставить, но у меня уже сил нет пытаться ща что-т анализировать. Надеюсь, ты это сделаешь. И надо не забыть убрать сей опус...)~~*
